...
AI LOOKUPS: Each "Full AI Pricing" or "eBay Comps Only" action on the pricing page counts as one lookup. Solo users get 200 per month. Starter, Standard, and Pro users get unlimited lookups. Solo users can buy additional 50-lookup packs for $5 from the dashboard or Settings → Billing.
...
INVENTORY: The inventory page shows all items. Solo users see All/Priced/Sold/Archived tabs. Starter+ users see All/Pending/Priced/Sold/Donated tabs with consignor filter. Use "Mark as Sold" on priced items. Use "Price" on pending items to go to the pricing screen.

REPORTS: The reports page shows payout summaries per consignor, total inventory value, days-to-sell tracking, and CSV export. Consignor payout = their split % × sold price for each item sold. Reports require Starter tier or above.

SETTINGS: Solo users see Billing and Profile tabs. Billing shows lookup usage, buy more, manage subscription, and upgrade options. Profile allows editing your name and changing your password. Starter+ users see Location Settings (store name, address, split %, agreement days, grace days, markdown toggle), Account Settings (account name, tier, billing, team management), and Locations (multi-location management for Standard+).

TIERS: Solo ($9/mo) — pricing only, 200 lookups. Starter ($49/mo) — adds consignors, lifecycle, reports, payouts, markdowns, staff. Standard ($79/mo) — adds multi-location, repeat item history, email notifications. Pro ($129/mo) — adds cross-customer pricing, community feed, All Locations dashboard, API access.
...
MULTI-LOCATION: If you have multiple store locations, use the location switcher in the sidebar to switch between them. Owners can see all locations. Staff are locked to their assigned location. Owners can see an All Locations aggregate view on the dashboard. Multi-location requires Standard tier or above.
