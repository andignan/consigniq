    ...
    <div>
      ...
      <div className="flex gap-2 items-start flex-wrap">
          ...
          <div key={photo.id} className="relative group">
            <div className={`${slotSize} rounded-xl overflow-hidden border border-gray-200 bg-gray-50 relative`}>
              ...
              {index === 0 && photos.length > 1 && (
                <span className="absolute bottom-1 left-1 text-[9px] font-bold bg-brand-500 text-white px-1.5 py-0.5 rounded-full">
                  Primary
                </span>
              )}
              {/* Error badge */}
        ...
        {canAdd && (
          <button
            ...
            className={`${slotSize} flex flex-col items-center justify-center gap-1 border-2 border-dashed border-gray-200 rounded-xl text-gray-400 hover:border-brand-300 hover:text-brand-500 transition-colors`}
          >
      ...
      {photos.length > 0 && (
        <button
          ...
          className="mt-3 flex items-center gap-2 px-4 py-2 text-sm font-medium text-brand-600 bg-brand-50 hover:bg-brand-100 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl transition-colors"
        >
          {analyzing ? (
            <>
